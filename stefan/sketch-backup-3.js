...
function getSnippet(text, query) {
  if (!query) return "";
  const regex = new RegExp(`\\b(\\w+)\\s+(${query})\\s+(\\w+)\\b`, "i");
  ...
  // Check if query is at beginning or end
  const startRegex = new RegExp(`^(${query})\\s+(\\w+)\\b`, "i");
  const endRegex = new RegExp(`\\b(\\w+)\\s+(${query})$`, "i");
  ...
  if (endRegex.test(text)) {
    const m = text.match(endRegex);
    return `${m[1]} ${m[2]}`;
  }

  return ""; // no exact match snippet
}
